    ...
    <>
      <div className="flex items-start px-6 pt-8 pb-6 md:px-12 md:pt-10 lg:px-24 lg:pt-12 lg:pb-8">
        {headerHref ? (
          <Link
            href={headerHref}
            aria-label={headerLinkLabel ?? `${clubName} 상세로 이동`}
            className="rounded-[20px] transition-opacity hover:opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-4"
          >
      ...
      {(oneLiner || (buttonText && buttonHref)) && (
        <div className="flex flex-col items-center gap-4 px-6 py-6 md:flex-row md:justify-center md:gap-6 md:px-12 md:py-8 lg:gap-8 lg:px-24 lg:py-10">
          ...
          {buttonText && buttonHref && (
            <Link
              href={buttonHref}
              className="w-full flex-shrink-0 rounded-[10px] bg-primary-500 px-8 py-3 text-center font-medium text-[14px] text-white hover:bg-primary-700 hover:text-gray-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-4 md:w-auto md:px-12 md:py-3.5 md:text-[15px] lg:px-20"
            >
              {buttonText}
            </Link>
          )}
