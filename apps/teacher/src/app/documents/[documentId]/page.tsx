...
import { getTeacherOnePagerDetail, getTeacherOnePagerForm } from "@/api/teacher";
import type { TeacherOnePagerDetailResponse, TeacherOnePagerFormResponse } from "@/types/teacher";
...
const STATUS_STYLES: Record<OnePagerFileStatus, { border: string; text: string }> = {
...
export default function TeacherDocumentDetailPage() {
  ...
  const [readOnlyData, setReadOnlyData] = useState<TeacherOnePagerFormResponse | null>(null);
  ...
  const openStatusModal = (idx: number) => {
    ...
    setPendingStatus(STATUS_MAP[target.onePagerState] || DEFAULT_ONE_PAGER_FILE_STATUS);
  };
  ...
  const title = (isOwner ? data?.title : readOnlyData?.title) || DEFAULT_TITLE;
  const description = (isOwner ? data?.description : readOnlyData?.description) || DEFAULT_DESCRIPTION;
  
  const currentFileUrl = isOwner ? data?.fileUrl : readOnlyData?.fileUrl;
  const currentFormUrl = isOwner ? data?.formUrl : readOnlyData?.formUrl;
  
  const source = currentFileUrl || currentFormUrl || "";
  const sourceType = currentFileUrl ? "file" : "url";
  const sourceName = sourceType === "file" ? "원페이저 양식.pdf" : "원페이저 링크";

  const durationType = isOwner ? "DATE" : readOnlyData?.onePagerDurationType;
  const duration = isOwner ? data?.onePagerDuration : readOnlyData?.onePagerDuration;
  const deadlineText = durationType === "INFINITY" || !duration
    ? "마감기한 없음"
      ...
      <div className="mx-auto max-w-7xl px-6">
        <div
          className={`grid gap-14 ${
            isOwner && submissions.length > 0 ? "lg:grid-cols-[1fr_480px]" : "grid-cols-1"
          }`}
        >
              ...
              {isOwner && (
                <div className="relative" ref={menuRef}>
                  <button
                    ...
                    className="p-1 text-[#989898] hover:text-gray-600 transition-colors"
                  >
                    <svg
                      width="24"
                      height="24"
                  ...
                  {isMenuOpen && (
                    <div className="absolute right-0 mt-2 w-[160px] rounded-[30px] bg-white p-5 shadow-[0_4px_24px_rgba(0,0,0,0.1)] border border-gray-50 z-10 flex flex-col gap-3">
                      <button
                        type="button"
          ...
          {isOwner && submissions.length > 0 && (
            <section className="flex flex-col gap-6">
              {currentSubmissions.map((sub, i) => {
                ...
                const displayStatus = STATUS_MAP[sub.onePagerState] || "제출됨";
                return (
                  <article
                    key={`${sub.clubName}-${sub.submitDate}`}
                    ...
                    <div className="mb-6 flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`rounded-full border px-5 py-1.5 font-bold text-[15px] ${
                          STATUS_STYLES[displayStatus]?.border || "border-gray-300"
                    ...
                    <div className="mb-2 flex items-center">
                      <div className="flex items-center gap-2 rounded-full border border-[#D66A6A] bg-white px-4 py-2 text-gray-800">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            handleDownload(sub.submitFileUrl, `${sub.clubName}_제출자료.pdf`)
                          }
                      ...
                      {sub.submitComments.length > 0 && (
                        <div className="mb-5 space-y-4">
                          {sub.submitComments.map((comment, cIdx) => (
                            <div key={`${comment.commentWriter}-${cIdx}`} className="flex gap-3">
